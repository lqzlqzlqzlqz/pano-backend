...
import { PanosService } from '../../service/sys/panos';
...
export class BaseOpenController extends BaseController {
  ...
  /**
   * 全景图，通过 project_id和pano_id 获取全景图信息
   * @param getPanos
   */
  @CoolTag(TagTypes.IGNORE_TOKEN)
  @Post('/getPanos', { summary: '获取全景图信息' })
  async getPanos(@Body() info: PanoInfoDTO) {
    return this.ok(await this.panosService.getPanos(info));
  }
}
